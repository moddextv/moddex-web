...
describe('the badges are read off disk, not fetched', () => {
  it('reads the file beside og.png', () => {
    expect(card).toMatch(/readFile\(join\(PUBLIC, 'badges', dir, file\)\)/);
  });

  /**
   * The url comes from the api, so it builds a path from a remote value and both
   * segments have to be constrained. Taking only the last one was enough while
   * badges were flat; after they moved into a directory each it would have read
   * `badges/icon.svg`, which exists for no badge — so every card would have
   * silently drawn none.
   */
  it('constrains both segments of the url the api answered with', () => {
    expect(card).toContain('/^[a-z0-9-]+$/.test(dir)');
    expect(card).toContain("file !== 'icon.svg'");
  });

  it('every badge on disk is where the card will look for it', () => {
    const dirs = readdirSync(join(ROOT, 'public', 'badges'), { withFileTypes: true })
    ...
    for (const dir of dirs) {
      ...
      expect(existsSync(join(ROOT, 'public', 'badges', dir, 'icon.svg'))).toBe(true);
    }
  });
});
