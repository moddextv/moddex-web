...
Everything below is _not done_. What is already done and verified is in the
other two documents.
...
- [x] **Set repo/CI values:** `IMAGE=ghcr.io/<owner>/<repo>` in the server
      ...
      **`NEXT_PUBLIC_PUBLISHABLE_SECRET_KEY` confirmed set on 2026-08-06**
      ...
      in the _server_ `.env` is still outstanding — that one is not a repo
...
**Apply `001` to production _before_ deploying the current code.** They are
coupled, and getting the order wrong breaks donations:

`audit`.`id` has no `AUTO_INCREMENT` in production, so
`INSERT INTO audit (type, message)` fails. Under the _old_ `db.query` that
...
- [x] **Stripe webhook.** **Done** — `POST /api/stripe/webhook` verifies the
      ...
      for handled _and_ for deliberately ignored events.
      `donationExists` no longer swallows database errors — it used to return
      "no such donation" when the _check_ failed, which under retries is how
...
- [x] **Apply `004-rebrand-chat-badge-labels.sql`.** **Confirmed applied
      ...
      `"Modchecker ..."` — they
      are _data_, so the code rebrand does not touch them and the old brand is
...
- [x] **N+1 in `getUsers`** — a badge query per user in a loop. **Done
      ...
      _downgraded_ it, overwriting the join's `{id, name, path}` with
...
- [ ] The two lockup SVGs in `public/logo/` reference Cairo 700 as a live font,
      ...
      Illustrator and `logo.ai` carried the _old_ wordmark anyway — so the
