...
const loadBadge = async (url: string): Promise<string | null> => {
  const [dir = '', file] = url.split('/').slice(-2);

  // both segments are constrained, because this builds a path from a remote value
  if (!/^[a-z0-9-]+$/.test(dir) || file !== 'icon.svg') return null;

  try {
    const svg = await readFile(join(PUBLIC, 'badges', dir, file));
