...
export const getChannelMods = async (user: User): Promise<User[]> => {
  return getUsersByRole(user, 'mods');
};

export const getChannelVips = async (user: User): Promise<User[]> => {
  return getUsersByRole(user, 'vips');
};


const getUsersByRole = async (user: User, role: Role): Promise<User[]> => {
  if (user.updated) {
...
const getAndStoreUsers = async (id: string, role: Role): Promise<User[]> => {
  const usersFromApi = role === 'mods' ? await fetchMods(id) : await fetchVips(id);
...
const storeUsers = async (id: string, users: User[], role: Role) => {
  ...
  for (const user of users) {
    const grantedDateString = new Date(user.granted as string).toISOString().slice(0, 19).replace('T', ' ');
    await db.query(
